...
/// @notice Headline scenario: bot opens + closes in the same block, pays ~90% tax.
...
contract JITScenarioTest is Test {
    ...
    function test_sameBlockJIT_pays90pct() public pure {
        uint256 taxBps = TaxCurve.calculateFinalTaxBps(0, CONCENTRATION_BPS);
        // With 91% concentration: multiplier = 1 + (0.91-0.5)*2 = 1.82
        // base = 9000, final = min(9000*1.82, 9900) = 9900
        assertEq(taxBps, 9900, "JIT bot with extreme concentration should hit 99% cap");

        uint128 taxAmount = uint128((uint256(BOT_FEES) * taxBps) / 10000);
        uint128 botKeeps = BOT_FEES - taxAmount;

        // Bot should keep < 10% of fees
        assertLt(botKeeps, BOT_FEES / 10, "Bot should keep less than 10% of fees");

        // Redistribution pool should receive > 90%
        assertGt(taxAmount, (BOT_FEES * 90) / 100, "Redistribution should receive > 90%");
    }
